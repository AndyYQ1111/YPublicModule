...
//  YPublicModuleDemo
//
//  Created by YueAndy on 2018/5/29.
...
@objc protocol StarViewDelegate{
    //返回星星评分的分值
    @objc optional func starRate(view starRateView:StarView,score:Float) -> ()
}
...
class StarView: UIView {
    ...
    var allowUserPan:Bool{//defualt is false,true为滑动评分
        set{
            if newValue {
                let pan = UIPanGestureRecognizer(target: self,action: #selector(StarView.starPan(_:)))
    ...
    fileprivate func createStarView()->(){
        ...
        //添加手势
        let tap = UITapGestureRecognizer(target: self,action: #selector(StarView.starTap(_:)))
...
//使用
/*
let starView = StarView.init(frame: CGRect(x: 20,y: 60,width: 300,height: 50), starCount: 6, score: 3.4)
